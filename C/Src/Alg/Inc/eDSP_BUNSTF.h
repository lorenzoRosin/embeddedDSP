...
#ifndef EDSP_BUNSTF_H
#define EDSP_BUNSTF_H
...
typedef enum
{
    e_eDSP_BUNSTF_RES_OK = 0,
    e_eDSP_BUNSTF_RES_BADPARAM,
    e_eDSP_BUNSTF_RES_BADPOINTER,
	e_eDSP_BUNSTF_RES_CORRUPTCTX,
    e_eDSP_BUNSTF_RES_OUTOFMEM,
    e_eDSP_BUNSTF_RES_BADFRAME,
	e_eDSP_BUNSTF_RES_FRAMEENDED,
    e_eDSP_BUNSTF_RES_FRAMERESTART,
    e_eDSP_BUNSTF_RES_NOINITLIB,
}e_eDSP_BUNSTF_RES;
 ...
 *
 * @return      e_eDSP_BUNSTF_RES_BADPOINTER   - In case of bad pointer passed to the function
 *		        e_eDSP_BUNSTF_RES_BADPARAM     - In case of an invalid parameter passed to the function
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_InitCtx(t_eDSP_BUNSTF_Ctx* const p_ptCtx, uint8_t* p_puBuff, const uint32_t p_uBuffL);
 ...
 *              e_eDSP_BUNSTF_RES_OK            - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_IsInit(t_eDSP_BUNSTF_Ctx* const p_ptCtx, bool_t* p_pbIsInit);
 ...
 *
 * @return      e_eDSP_BUNSTF_RES_BADPOINTER   - In case of bad pointer passed to the function
 *		        e_eDSP_BUNSTF_RES_NOINITLIB    - Need to init context before taking some action
 *		        e_eDSP_BUNSTF_RES_CORRUPTCTX   - In case of an corrupted context
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_NewFrame(t_eDSP_BUNSTF_Ctx* const p_ptCtx);
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_GetUnstufData(t_eDSP_BUNSTF_Ctx* const p_ptCtx, uint8_t** p_ppuData, uint32_t* const p_puL);
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_GetUnstufLen(t_eDSP_BUNSTF_Ctx* const p_ptCtx, uint32_t* const p_puL);
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_IsWaitingSof(const t_eDSP_BUNSTF_Ctx* p_ptCtx, bool_t* const p_pbIsWaitSof);
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_IsAFullFrameUnstuff(const t_eDSP_BUNSTF_Ctx* p_ptCtx, bool_t* const p_pbIsFrameUnstuff);

/**
 * @brief       Check if the current received data compose a bad frame. If a bad frame is detected we can only
 *              call eDSP_BUNSTF_NewFrame before parsing new data.
 *
 * @param[in]   p_ptCtx            - Byte unStuffer context
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_IsFrameBad(const t_eDSP_BUNSTF_Ctx* p_ptCtx, bool_t* const p_pbIsFrameBad);
 ...
 *                                  ( e_eDSP_BUNSTF_RES_FRAMEENDED, e_eDSP_BUNSTF_RES_BADFRAME or
 *                                  e_eDSP_BUNSTF_RES_FRAMERESTART is returned ) or when some other error is returned.
 *                                  When the function return e_eDSP_BUNSTF_RES_OK p_puConsumedBufL will always be
 ...
 *		        e_eDSP_BUNSTF_RES_BADPARAM     - In case of an invalid parameter passed to the function
 *		        e_eDSP_BUNSTF_RES_CORRUPTCTX   - In case of an corrupted context
 *              e_eDSP_BUNSTF_RES_OUTOFMEM     - Can not unstuff data, initial mem pointer was too small. The only way to
 *                                              resolve the issue is increasing the size of the memory area passed to
 *                                              init.
 *		        e_eDSP_BUNSTF_RES_FRAMEENDED   - Frame ended, restart context in order to parse a new frame. Every other
 *                                              call to this function will not have effect until we call
 *                                              eDSP_BUNSTF_NewFrame. In this situation bear in mind that some data
 *                                              could be left out the parsing, and so we need to reparse that data after
 *                                              calling eDSP_BUNSTF_NewFrame.
 *              e_eDSP_BUNSTF_RES_BADFRAME     - Found an error while parsing, the frame passed is invalid.
 *                                              Restart context in order to parse a new frame. Every other call
 *                                              to this function will not have effect until we call
 *                                              eDSP_BUNSTF_NewFrame. In this situation bear in mind that some data
 *                                              could be left out the parsing, and so we need to reparse that data after
 *                                              calling eDSP_BUNSTF_NewFrame.
 *              e_eDSP_BUNSTF_RES_FRAMERESTART - During frame receiving another start of frame is received. In this
 ...
 *              e_eDSP_BUNSTF_RES_OK           - Operation ended correctly. The chunk is parsed correclty but the frame
 *                                              is not finished yet. In this situation p_puConsumedBufL is always
 *                                              reported with a value equals to p_uStuffBufL.
 */
e_eDSP_BUNSTF_RES eDSP_BUNSTF_InsStufChunk(t_eDSP_BUNSTF_Ctx* const p_ptCtx, const uint8_t* p_puStuffBuf,
