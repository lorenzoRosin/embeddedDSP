	...
	*/
	...
	else
	{
		...
		else
		{
			...
			{
				/* Insert data */
				p_ptCtx->piWindowsBuffer[p_ptCtx->iPrevOutValue] = p_iValue;
				p_ptCtx->iPrevOutValue++;

				/* Manage rolback */
				if( p_ptCtx->iPrevOutValue >= p_ptCtx->uWindowsLen )
				{
					p_ptCtx->iPrevOutValue = 0u;
				}
				...
				if( p_ptCtx->uFilledData < ( p_ptCtx->uWindowsLen - 1u ) )
				{
					...
					/* Need more data */
					l_eRes = e_eDSP_FOLPASSFILTER_RES_NEEDSMOREVALUE;
				}
				else
				{
					/* The window is full */
					...
					while( ( e_eDSP_FOLPASSFILTER_RES_OK == l_eRes ) && ( l_uCnt < p_ptCtx->uWindowsLen ) )
					{
						l_eMaxRes = eDSP_MAXCHECK_SUMI64Check(l_iSum, p_ptCtx->piWindowsBuffer[l_uCnt]);
						l_eRes = eDSP_FOLPASSFILTER_MaxCheckResToFOLPF(l_eMaxRes);

						if( e_eDSP_FOLPASSFILTER_RES_OK == l_eRes )
						{
							l_uCnt++;
...
static bool_t eDSP_FOLPASSFILTER_IsStatusStillCoherent(t_eDSP_FOLPASSFILTER_Ctx* const p_ptCtx)
{
    ...
    else
    {
		...
		if( ( p_ptCtx->uWindowsLen <= 2u ) || ( p_ptCtx->uFilledData > p_ptCtx->uWindowsLen ) ||
			( p_ptCtx->iPrevOutValue >= p_ptCtx->uWindowsLen )  )
		{
			l_eRes = false;
		}
		else
		{
			/* Check data validity */
			if( ( p_ptCtx->uFilledData < p_ptCtx->uWindowsLen ) && ( p_ptCtx->iPrevOutValue >= p_ptCtx->uFilledData ) )
			{
				l_eRes = false;
			}
			else
